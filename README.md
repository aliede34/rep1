...
### Derleyici Araçlarını Kurma

- Debian/Ubuntu tabanlı sistemlerde:

  ```bash
  sudo apt update
  sudo apt install nasm binutils grub-pc-bin xorriso
  ```

- Arch Linux veya MSYS2 ortamlarında:

  ```bash
  pacman -S nasm binutils grub
  ```

`make` komutunun çıktısında "NASM assembler not found" benzeri bir hata alırsanız `nasm` aracının PATH içinde olduğundan emin olun.
